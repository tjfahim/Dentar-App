...
<div class="row mt-4">
    <div class="col-lg-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title">Edit Diabetic Guide</h4>
                <form action="{{ route('diabetic.update', $guide->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group mt-3">
                        ...
                        <small>Current File: <a href="{{ asset($guide->file) }}" target="_blank">View File</a></small>
                        @error('file')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <button type="submit" class="btn btn-primary mt-3">Update Diabetic Guide</button>
                    <a href="{{ route('diabetic.index') }}" class="btn btn-secondary mt-3">Cancel</a>
                </form>
            </div>
        </div>
    </div>
